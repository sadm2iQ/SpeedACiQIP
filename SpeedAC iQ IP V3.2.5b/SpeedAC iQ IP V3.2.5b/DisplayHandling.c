...
//Refresh Auto Screen 
    if (RefreshAutoScreenFlag != Done && Current_Screen == SCREEN_AUTOMATIC && T_Dn(T_FirstScan) && !BootUp)  //SADM2 2020 NOVEMBER
    {  
      BootUp = ON;
      RefreshAutoScreen(OFF);
      RefreshAutoScreenFlag = Done;
    }
    if (RefreshAutoScreenFlag != Done && Current_Screen == SCREEN_AUTOMATIC && T_Dn(T_FirstScan) && EmptyWeightReady)  //SADM2 2020 NOVEMBER
    {  
      BootUp = ON;
      RefreshAutoScreen(OFF);
      RefreshAutoScreenFlag = Done;
