...
void Display_AdditionalLiveInfo(void)
{
  int DW_Pointer; 
     ...
     DW_Pointer++;
     return_value[DW_Pointer] = WP.TargetWeightIP;             //56  
     DW_Pointer++;
     return_value[DW_Pointer] = ThroughputAutoPage;            //58
...
int DW_Pointer; 
     ...
     return_value[DW_Pointer] = Read_Auto_Page; //Value32  //14 
     DW_Pointer++;
     return_value[DW_Pointer] = WP.No;                     //16
     DW_Pointer++;                                     
     return_value[DW_Pointer] = TargetWeight[Scale1];      //18
     DW_Pointer++;
     return_value[DW_Pointer] = ProdAvgWeight[Scale1];     //20
     DW_Pointer++;
     return_value[DW_Pointer] = ThroughputAutoPage;        //22
     DW_Pointer++;
     return_value[DW_Pointer] = ProdTotalUnitsCnt[Scale1]; //24
     DW_Pointer++;
     return_value[DW_Pointer] = ProdBulkDuration[Scale1];  //26
     DW_Pointer++;
     return_value[DW_Pointer] = ProdDribbleDuration[Scale1];   //28
     DW_Pointer++; 
     return_value[DW_Pointer] = ProdCycleDuration[Scale1];     //30  
}
               ...
               if(IN_ValueMem == IN_Value && IN_CommandMem == IN_Command && IN_ParameterMem == IN_Parameter)
                ...
                {             
                         switch (IN_Parameter)
                         {
                          ...
                          case Change_Target:
                            if (IN_Value != NominalWeightMem)
                              {
                                TargetPLC = ON;
                                NominalWeightMem = IN_Value;
                                return_value[0] = Change_Target;
                              }
                              else return_value[0] = Change_Target + InvalidNbr;
                                break; 
                          
                          case Change_TargetIP:
                            if (IN_Value != WP.TargetWeightIP && !Recipe_Grayout[15])
                              {
                                TargetWeighIPEntry(string(IN_Value));
                                return_value[0] = Change_TargetIP;
                              }
                              else return_value[0] = Change_TargetIP + InvalidNbr;
                                break;           
