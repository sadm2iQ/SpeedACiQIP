...
//Refresh Auto Screen 
    if (RefreshAutoScreenFlag != Done && Current_Screen == SCREEN_AUTOMATIC && T_Dn(T_FirstScan) && (!BootUp || EmptyWeightReady || PLCSend))  //SADM2 2020 NOVEMBER
    {  
      if(!BootUp)
       BootUp = ON;
      if(EmptyWeightReady)
       EmptyWeightReady = OFF;
      if(PLCSend)
       PLCSend = OFF; 
